...
### Repository
- Single shared repository: `aramark-destinations/aramark-mb` (repo root = EDS root)
- All properties share the same core block library
- Brand differences handled through tokens, not custom code

### Branch-to-Environment Mapping
- `main` → Production (`https://main--aramark-mb--aramark-destinations.aem.live/`)
- `staging` → Staging (`https://staging--aramark-mb--aramark-destinations.aem.page/`)
- Feature branches → Preview (`https://{branch}--aramark-mb--aramark-destinations.aem.page/`)

Branch naming convention: `ADO-{ticket}-{block/element/feature}` (e.g., `ADO-94-cards`, `ADO-120-carousel-scroll`)

### Content Mountpoints

Each brand's AEM content tree maps to the web root (`/`) via a path mapping registered via the `admin.hlx.page` config API. The `/brands/{brand}/` path is the local **code** directory only — not a URL path.

| AEM Content Path | Web Root | Brand Code Directory |
|---|---|---|
| `/content/aramark-mb` | `/` | `brands/unbranded/` |
| `/content/lake-powell` | `/` | `brands/lake-powell/` |
| `/content/travel-yosemite` | `/` | `brands/travel-yosemite/` |

Each new brand requires a site registration via the admin API (no new repo or fork needed). See `docs/BRAND-SETUP-GUIDE.md`.

## Token Governance (Planned)

A future workflow will manage Root token updates through AEM Universal Editor forms → Adobe App Builder CI pipeline → auto PR to staging. This is defined in `docs/in-progress/FED-SOLUTION-DESIGN.md` and is not yet implemented. Currently, root tokens are edited directly in `styles/root-tokens.scss`.
