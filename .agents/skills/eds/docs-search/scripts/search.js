...
const LOCAL_DOCS_DIR = path.join(PROJECT_ROOT, 'docs');
...
/**
 * Parse a local markdown file into a searchable doc object
 */
function parseLocalDoc(filePath) {
  ...
  // Extract description: first non-empty, non-heading, non-code, non-table paragraph after the title
  let description = '';
  ...
  for (const line of lines) {
    ...
    if (!seenTitle) {
      if (/^#\s/.test(trimmed)) seenTitle = true;
      continue;
    }
    if (
      !trimmed
  ...
  // Strip markdown syntax for content scoring
  const content = raw
  ...
  return { path: relativePath, title, description, content, deprecation: null };
}
...
/**
 * Search local docs/ markdown files (synchronous, no caching needed)
 */
function searchLocalDocs(keywords) {
  const files = collectMarkdownFiles(LOCAL_DOCS_DIR);
  const docs = files.map(parseLocalDoc);
  return searchDocuments(docs, keywords, 'local');
}
...
async function search(keywords, limit = 10) {
  ...
  // Run local search (sync) and remote fetch (async) concurrently
  const localResults = searchLocalDocs(filteredKeywords);
  ...
  // Search docpages and merge deprecation data
  const docpagesData = docpagesIndex.data.map(doc => ({
    ...doc,
    deprecation: deprecationMap.get(doc.path) || doc.deprecation || null
  }));
  let remoteResults = searchDocuments(docpagesData, filteredKeywords, 'doc');

  // If fewer than 5 remote doc results, search blog posts too
  if (remoteResults.length < 5) {
    ...
    remoteResults = [...remoteResults, ...blogResults].sort((a, b) => b.relevanceScore - a.relevanceScore);
  }

  // Merge all results sorted by relevance score
